...
#
# Tags look like `v<Docmost version>-<release>`:
#
# - if defaults/main.yml points at a Docmost version that has never been
#   released, the release counter restarts at 0 (`v0.95.0-0`)
# - otherwise the counter is incremented (`v0.95.0-1`), but only if something
...
# Anchored on `docmost_version:` so that neither the similarly named
# `docmost_container_image_self_build_repo_version:` nor a commented-out line
# can be mistaken for it.
version="$(sed -nE 's|^docmost_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Docmost version from $defaults_path"
	exit 1
fi

# Docmost's version is carried without a leading `v` (e.g. `0.95.0`) while the
# tags have one, so it is added here rather than doubled.
